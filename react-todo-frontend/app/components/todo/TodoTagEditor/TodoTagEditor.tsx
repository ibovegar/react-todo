...
export function TodoTagEditor(props: TodoTagEditorProps) {
  const { tags, availableTags, onAdd, onRemove } = props
  const unusedTags = useMemo(() => getUnusedTags(availableTags, tags), [availableTags, tags])
  ...
  return (
    <HStack gap="space-2" wrap align="center">
      ...
      {unusedTags.length > 0 && <TagPicker unusedTags={unusedTags} hasExistingTags={tags.length > 0} onAdd={onAdd} />}
    </HStack>
  )
}
