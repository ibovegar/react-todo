...
export function CreateTodoCard(props: CreateTodoCardProps) {
  ...
  if (!animation.isOpen) {
    return (
      ...
      >
        <VStack gap="space-4" align="center" justify="center" style={{ height: '100%' }}>
          <PlusIcon fontSize="2rem" style={{ color: 'var(--ax-text-neutral-subtle)' }} />
          <Heading size="small" style={{ color: 'var(--ax-text-neutral-subtle)' }}>
            New todo
          </Heading>
        </VStack>
      </Box>
    ...
    >
      <fetcher.Form method="post">
        ...
        <VStack gap="space-16">
          ...
          <Textarea label="Description" name="description" size="small" minRows={8} />
          {availableTags.length > 0 && (
            <VStack gap="space-4">
              <Heading size="xsmall">Tags</Heading>
              <TagCloud
                tags={availableTags}
                disabledTags={availableTags.filter((t) => !selectedTags.some((s) => s.name === t.name))}
                onSelect={(tag) => {
                  const isSelected = selectedTags.some((s) => s.name === tag.name)
                  setSelectedTags((prev) => (isSelected ? prev.filter((t) => t.name !== tag.name) : [...prev, tag]))
                }}
              />
            </VStack>
          )}
          <HStack gap="space-4" justify="end">
            <Button type="button" variant="tertiary" size="small" onClick={animation.close}>
              Cancel
            </Button>
